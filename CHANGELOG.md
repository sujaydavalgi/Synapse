...
- **Interpreter Phase 8 (partial):** runtime tree routes AST, foundations, comm/robotics decls, and robotics runtime state through `spanda-ast` / `spanda-runtime` instead of `crate::ast` / `crate::foundations`; `lean_core_shims` guard added; follow-up routes audit, providers, security, scheduler, robot state, connectivity geofences, transport security, and triggers through workspace crates; **`TriggerRegistry` moved to `spanda-runtime`** with a thin `spanda-core` shim.
