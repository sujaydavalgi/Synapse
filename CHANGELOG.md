...
- **Persistent telemetry store:** session/run linking with mission traces; end-of-run `runtime_metrics` snapshots; JSONL retention via `SPANDA_TELEMETRY_MAX_EVENTS`; TS CLI JSONL fallback; prior batch: `robot_id`, topic publish, fleet flag, deploy agent heartbeats, golden path, `spanda man telemetry`
