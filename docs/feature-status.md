...
| **Enterprise operations (NEXT)** | Native tonic gRPC (JSON-RPC gateway exists), full multi-dimension configuration drift, live discovery transports (mDNS/USB/BLE packages), vendor alert channel packages, OTLP metrics export, production desktop installers |
